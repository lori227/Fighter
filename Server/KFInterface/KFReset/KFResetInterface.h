...
namespace KFrame
{
    typedef std::function< void( KFEntity*, uint32, const KFTimeData*, uint64, uint64 ) > KFResetFunction;
    class KFResetInterface : public KFModule
    {
    public:
        // 注册重置逻辑
        template< class T >
        void RegisterResetFunction( uint32 timeid, uint32 count, T* object, void ( T::*handle )( KFEntity*, uint32, const KFTimeData*, uint64, uint64 ) )
        {
            KFResetFunction function = std::bind( handle, object, std::placeholders::_1, std::placeholders::_2,
...
#define __KF_RESET_FUNCTION__( function ) void function( KFEntity* player, uint32 timeid, const KFTimeData* timedata, uint64 lasttime, uint64 nowtime )
